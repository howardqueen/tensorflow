...
# 获取损失
def loss(loss_object, model, x, y):
    y_ = model(x)
    return loss_object(y_true=y, y_pred=y_)
    
def grad(loss_object, model, inputs, targets):
    with tf.GradientTape() as tape:
        loss_value = loss(loss_object, model, inputs, targets)
...
def keras_train():
    ...
    l = loss(loss_object, model, features, labels)
    print(l)

    
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.01)

    loss_value, grads = grad(loss_object, model, features, labels)
    print('步数：{}, 初始loss值：{}'.format(optimizer.iterations.numpy(),
                                    loss_value.numpy()))
    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    print('步数：{}, loss值：{}'.format(optimizer.iterations.numpy(),
                                    loss(loss_object, model,features, labels).numpy()))
